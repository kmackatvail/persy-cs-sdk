...
namespace com.persephony.api.call
{
    ...
    public sealed class CallsRequester : APIRequester
    {
        ...
        /// <returns>The call matching the callId provided.</returns>
        /// <exception cref="PersyException">Thrown upon failed request.</exception>
        public Call update(string callId, CallTermOptions callOptions = null)
        {
            string json = base.POST(String.Format("{0}/{1}", this.path, callId), ((callOptions != null) ? callOptions.toJson() : null));

            if (string.IsNullOrEmpty(json) == true)
            {
                throw new PersyException(String.Format("Failed to update call {0} information", callId ?? ""));
            }
